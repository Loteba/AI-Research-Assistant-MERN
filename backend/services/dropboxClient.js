...
const fetchAccessToken = async () => {
  ...
  // 2. Si existe un token de acceso estático (útil en tests o despliegues sencillos), úsalo
  const staticAccess = (process.env.DROPBOX_ACCESS_TOKEN || '').trim();
  if (staticAccess) {
    cachedToken = staticAccess;
    // dejamos expiry corto para forzar refresco en próximas llamadas si procede
    cachedExpiry = Date.now() + 60 * 60 * 1000;
    return cachedToken;
  }

  // 3. Obtiene las credenciales de las variables de entorno para refrescar
